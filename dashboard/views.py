    ...
    def get(self, request, pk):
        try:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="Factura-{factura.numero_factura}.pdf"'
            )
        ...
        try:
            ...
            for row_number, row in enumerate(reader, start=1):
                try:
                    ...
                    if not proveedor:
                        errores.append(
                            f"Fila {row_number}: Proveedor '{row['proveedor']}' no encontrado"
                        )
                        continue
            ...
            if errores:
                return Response(
                    {"error": "Errores al procesar el archivo", "detalles": errores},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            return Response(
                {"mensaje": "Facturas importadas correctamente"},
                status=status.HTTP_201_CREATED,
            )
        except Exception as e:
            return Response(
                {"error": f"Error procesando el archivo CSV: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        data = [
            {
                ...
                "facturas": FacturaSerializer(
                    proveedor.factura_set.all(), many=True
                ).data,
            }
...
class ActualizarEstadoFacturaProveedor(APIView):
    ...
    def patch(self, request, pk):
        ...
        factura.save()
        return Response(
            {"mensaje": "Estado actualizado correctamente"}, status=status.HTTP_200_OK
        )
...
class NotificacionesView(APIView):
    ...
    def get(self, request):
        ...
        if request.user.rol not in ["Contador", "Administrador"]:
            return Response(
                {
                    "error": "No autorizado. Solo los contadores y administradores pueden acceder."
                },
        ...
        facturas_vencidas = Factura.objects.filter(
            fecha_vencimiento__lt=today, estado="Pendiente"
        )
...
class EstadisticasDashboardView(APIView):
    ...
    def get(self, request):
        ...
        if request.user.rol not in ["Administrador", "Gerente"]:
            return Response(
                {
                    "error": "No autorizado. Solo Administradores y Gerentes pueden acceder."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        facturas_pagadas_clientes = Factura.objects.filter(
            cliente__isnull=False, estado="Pagada"
        ).count()
        facturas_pagadas_proveedores = Factura.objects.filter(
            proveedor__isnull=False, estado="Pagada"
        ).count()
        ...
        facturas_vencidas = Factura.objects.filter(
            estado="Vencida", fecha_vencimiento__month=today.month
        ).count()
        comparacion_pagos = {
            "pagos": Factura.objects.filter(
                proveedor__isnull=False, estado="Pagada"
            ).aggregate(total=Sum("monto_total"))["total"]
            or 0,
            "cobros": Factura.objects.filter(
                cliente__isnull=False, estado="Pagada"
            ).aggregate(total=Sum("monto_total"))["total"]
            or 0,
        }
...
class FacturasVencidasCountView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"facturas_vencidas_count": facturas_vencidas_count}, status=200
        )
